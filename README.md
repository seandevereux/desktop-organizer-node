...
- Watch for changes in the Angular renderer
- Automatically restart Electron when changes are detected

**Option 2: Manual (More Control)**

Terminal 1 - Build main process and preload (watch mode):
...
Terminal 2 - Build Angular renderer (watch mode):
...
Note: In development mode, the app will connect to `http://localhost:4200` for the Angular app. Make sure the Angular dev server is running or use the build:renderer watch mode.
...
1. **Create your icon files:**
   ...
   - For Windows: Create an `.ico` file (recommended sizes: 16x16, 32x32, 48x48, 256x256)
   - For all platforms: Create a `.png` file (recommended size: 256x256 or 512x512)

2. **Place the icons:**

   - Save `icon.ico` in the `assets/` folder (for Windows)
   - Save `icon.png` in the `assets/` folder (for macOS/Linux and fallback)

3. **Icon creation tools:**

   - Online converters: https://convertio.co/png-ico/ or https://www.icoconverter.com/
   - Design tools: Figma, Photoshop, GIMP, or any image editor
   - For best results, create icons with multiple sizes embedded in the .ico file
